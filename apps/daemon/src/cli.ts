...
import { runBashToolCli } from './tools-bash-cli.js';
...
} else if (argv[0] === 'tools' && argv[1] === 'bash') {
  runBashToolCli(argv.slice(2))
    .then(({ exitCode }) => {
      process.exitCode = exitCode;
    })
    .catch((error) => {
...
function printRootHelp() {
  ...
  od tools bash --script <script> [options]
      Execute shell syntax in the project-scoped hosted just-bash workspace.
      The hosted workspace is in-memory and cannot invoke native processes.
...
Tools exposed:
  ...
  run_bash(command[, project])   run shell syntax in the in-memory hosted workspace
